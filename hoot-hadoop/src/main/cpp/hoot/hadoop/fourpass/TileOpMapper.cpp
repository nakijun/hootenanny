...
void TileOpMapper::_addNode(const boost::shared_ptr<Node>& n)
{
  long key = -1;
...
void TileOpMapper::_addWay(ConstOsmMapPtrR map, const boost::shared_ptr<Way>& w)
{
  long key = -1;
...
void TileOpMapper::_flush()
{
  ...
  // emit all maps
  for (QHash< int, boost::shared_ptr<PbfWriter> >::iterator it = _writers.begin();
    it != _writers.end(); ++it)
  {
    *k = it.key();
...
void TileOpMapper::_init(HadoopPipes::MapContext& context)
{
  ...
  for (size_t i = 0; i < _envelopes.size(); i++)
  {
    _writers[i] = boost::shared_ptr<PbfWriter>(new PbfWriter());
  ...
  {
    _writers[-1 - i] = boost::shared_ptr<PbfWriter>(new PbfWriter());
...
void TileOpMapper::_map(boost::shared_ptr<OsmMap>& m, HadoopPipes::MapContext& context)
{
  _init(context);
...
void TileOpMapper::_replaceNodes(boost::shared_ptr<OsmMap>& m)
{
  pp::Hdfs fs;
...
void TileOpMapper::_replaceNodes(boost::shared_ptr<OsmMap>& m, istream& is)
{
  int64_t ids[2];
  while (!is.eof())
  {
