...
public:
   ...
   */
  WaySplitter(const OsmMapPtr& map, boost::shared_ptr<Way> a);
   ...
   */
  vector< boost::shared_ptr<Way> > split(WayLocation& splitPoint);
   ...
   */
  static void split(const OsmMapPtr& map, const boost::shared_ptr<Way>& w, double maxSize);

  static vector< boost::shared_ptr<Way> > split(const OsmMapPtr& map, boost::shared_ptr<Way> a,
    WayLocation& splitPoint);

private:
  OsmMapPtr _map;
  boost::shared_ptr<Way> _a;
  auto_ptr<FindNodesInWayFactory> _nf;

  boost::shared_ptr<Node> _createNode(const Coordinate& c);
};
