  ...
  _map(map)
{
  ...
  else
  {
    ...
    {
      boost::shared_ptr<const Node> n = _map->getNode(_way->getNodeId(i));
...
Meters WayLocation::calculateDistanceOnWay() const
{
  ...
  if (_segmentIndex < (int)_way->getNodeCount() - 1)
  {
    boost::shared_ptr<const Node> n = _map->getNode(_way->getNodeId(_segmentIndex + 1));
    Coordinate next = n->toCoordinate();
...
{
  boost::shared_ptr<const Node> p0 = _map->getNode(_way->getNodeId(_segmentIndex));
  ...
  {
    boost::shared_ptr<const Node> p1 = _map->getNode(_way->getNodeId(_segmentIndex + 1));
...
WayLocation WayLocation::move(Meters distance) const
{
  ...
  while (1 + distance > 1)
  {
    ...
    boost::shared_ptr<const Node> n = _map->getNode(_way->getNodeId(result.getSegmentIndex() + 1));
    Coordinate next = n->toCoordinate();
