...
#ifndef TILEWORKER2_H
#define TILEWORKER2_H
...
public:
  ...
  /**
   * Applies an operation to all the elements that intersect an entry in "tiles".
   * @param op - The operation to apply.
   * @param tiles - The tiles to conflate. The tiles have not been expanded by buffer.
   * @param buffer - The size of the buffer. Generally, you'll have to buffer the tiles before using
   *        the bounds.
   ...
   * @param in2 Second input, guaranteed to not change throughout the life of the object. The second
   *        input may be empty in which case it should not be used. If present gets assigned a
   *        status of Unknown2.
